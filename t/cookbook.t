...
my $t = Test::Mojo->new(Cookbook => {sqlite => ':temp:', secrets => ['test_s3cret']});
$t->ua->max_redirects(10);

# No posts yet
$t->get_ok('/')->status_is(200)->text_is('title' => 'Cookbook')
  ->text_is('body > a' => 'New post')->element_exists_not('h2');

# Create a new post
$t->get_ok('/recipes/create')->status_is(200)->text_is('title' => 'New recipe')
  ->element_exists('form input[name=title]')
  ->element_exists('form textarea[name=body]');
$t->post_ok('/recipes' => form => {title => 'Testing', body => 'This is a test.'})
...
$t->get_ok('/recipes/1')->status_is(200)->text_is('title' => 'Testing')
  ->text_is('h2' => 'Testing')->text_like('p' => qr/This is a test/)
  ->text_is('body > a' => 'Edit');

# Update the post
$t->get_ok('/recipes/1/edit')->status_is(200)->text_is('title' => 'Edit recipe')
...
$t->post_ok(
  '/recipes/1?_method=PUT' => form => {title => 'Again', body => 'It works.'})
  ->status_is(200)->text_is('title' => 'Again')->text_is('h2' => 'Again')
  ->text_like('p' => qr/It works/);
$t->get_ok('/recipes/1')->status_is(200)->text_is('title' => 'Again')
  ->text_is('h2' => 'Again')->text_like('p' => qr/It works/);

# Delete the post
$t->post_ok('/recipes/1?_method=DELETE')->status_is(200)
  ->text_is('title' => 'Cookbook')->element_exists_not('h2');
